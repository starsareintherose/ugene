...
public:
    ...
    bool containsEmptyVars() const {
        return vars.values().contains(QString(""));
    }
...
public:
    ...
    bool isPassed() const {
        return state == TriState_Yes;
    }

    bool isFailed() const {
        return state == TriState_No;
    }
...
enum GTestLogHelperStatus { GTest_LogHelper_Invalid,
                            GTest_LogHelper_Valid };
